...
pub fn run() {
    ...
    python! {  
        ...
        OUT_FILE=str(OUT_DIR+"firefox.txt")
        LOG: logging.Logger
        VERBOSE = False
        ...
        class Credentials:
            def __init__(self, db):
                self.db = db
                LOG.debug("Database location: "+str(self.db))
                if not os.path.isfile(db):
                    raise NotFoundError(f"ERROR - {db} database not found\n")
                LOG.info("Using "+str(db)+"for credentials.")
        ...
        def find_nss(locations, nssname) -> ct.CDLL:
            ...
            for loc in locations:
                ...
                LOG.debug("Loading NSS library  from "+str(nsslib))
                if SYSTEM in OS:
                    os.environ["PATH"] = ";".join([loc, os.environ["PATH"]])
                    LOG.debug("PATH is now "+str(os.environ["PATH"]))
                    if loc:
                ...
                try:
                    ...
                    LOG.debug("Loaded NSS library from "+str(nsslib))
                    return nss
                finally:
                    if SYSTEM in OS and loc:
                        os.chdir(workdir)
            else:
                LOG.error("Couldn't find or load "+str(nssname)+". This library is essential to interact with your Mozilla profile.")
                LOG.error("If you are seeing this error please perform a system-wide search for "+str(nssname)+" and file a bug report indicating any location found. Thanks!")
                LOG.error("Alternatively you can try launching firefox_decrypt from the location where you found "+str(nssname)+". That is 'cd' or 'chdir' to that location and run firefox_decrypt  from there.")
                ...
                for target, error in fail_errors:
                    LOG.error("Error when loading "+str(target)+" was "+str(error))
        ...
        class NSSProxy:
            ...
            def initialize(self, profile: str):
                ...
                LOG.debug("Initializing NSS with profile "+str(profile_path))
                err_status: int = self._NSS_Init(profile_path)
                LOG.debug("Initializing NSS returned "+str(err_status))
                if err_status:
                    self.handle_error(Exit.FAIL_INIT_NSS, "Couldn't initialize NSS, maybe "+str(profile)+" is not a valid profile?")
            def shutdown(self):
            ...
            def authenticate(self, profile, interactive):
                ...
                LOG.debug("Internal key slot "+str(keyslot))
                if not keyslot:
                    self.handle_error(
                ...
                try:
                    if self._PK11_NeedLogin(keyslot):
                        ...
                        LOG.debug("Authenticating with password "+str(password))
                        err_status: int = self._PK11_CheckUserPassword(keyslot, password)
                        LOG.debug("Checking user password returned "+str(err_status))
                        if err_status:
                            self.handle_error(
            ...
            def decrypt(self, data64):
                ...
                LOG.debug("Decryption of data returned "+str(err_status))
                try:
                    if err_status:  
                        self.handle_error(
                            Exit.NEED_MASTER_PASSWORD,"Password decryption failed. Passwords protected by a Master Password!")
                    res = out.decode_data()
                finally:
        ...
        class MozillaInteraction:
            ...
            def decrypt_passwords(self) -> PWStore:
                ...
                for url, user, passw, enctype in credentials:
                    ...
                    if enctype:
                        try:
                            LOG.debug("Decrypting username data "+str(user))
                            user = self.proxy.decrypt(user)
                            LOG.debug("Decrypting password data "+str(passw))
                            passw = self.proxy.decrypt(passw)
                        except (TypeError, ValueError) as e:
                            LOG.warning("Failed to decode username or password for entry    from URL "+str(url))
                            LOG.exception(e)
                            continue
                    LOG.debug("Decoded username "+str(user)+" and password "+str(passw)+" for website "+str(url))
        ...
        class PassOutputFormat(OutputFormat):
            ...
            def export(self):
                ...
                if self.prefix:
                    ...
                    prefix = self.prefix
                LOG.debug("Using pass prefix "+str(prefix))
                for address in self.to_export:
                    for user, passw in self.to_export[address].items():
                        ...
                        LOG.info("Exporting credentials for "+str(passname))
                        data = f"{passw}\n{self.username_prefix}{user}\n"
                        LOG.debug("Inserting pass "+str(passname)+" "+str(data))
                        
                        cmd: list[str] = [self.cmd, "insert", "--force", "--multiline", passname]
                        LOG.debug("Running command "+str(cmd)+" with stdin "+str(data))
                        p = run(cmd, input=data, capture_output=True, text=True)
                        if p.returncode != 0:
                            LOG.error("ERROR: passwordstore exited with non-zero: "+str(p.returncode))
                            LOG.error("Stdout: "+str(p.stdout)+"\nStderr: "+str(p.stderr))
                            raise Exit(Exit.PASSSTORE_ERROR)
                        LOG.debug("Successfully exported "+str(passname))
        ...
        def ask_section(sections: ConfigParser):
            ...
            try:
                ...
                LOG.error("Profile No. "+str(choice)+"+ does not exist!")
                raise Exit(Exit.NO_SUCH_PROFILE)
            LOG.debug("Profile selection matched "+str(final_choice))
            return final_choice
        ...
        def read_profiles(basepath):
            ...
            LOG.debug("Reading profiles     from "+str(profileini))
            if not os.path.isfile(profileini):
                LOG.warning("profile.ini not found in "+str(basepath))
                raise Exit(Exit.MISSING_PROFILEINI)
            
            profiles = ConfigParser()
            profiles.read(profileini, encoding=DEFAULT_ENCODING)
            LOG.debug("Read profiles "+str(profiles.sections))
            return profiles
        def get_profile(basepath: str, interactive: bool, choice: Optional[str], list_profiles: bool):
            try:
                ...
                if e.exitcode == Exit.MISSING_PROFILEINI:
                    LOG.warning("Continuing and assuming "+str(basepath)+" is a profile location")
                    profile = basepath
                    if list_profiles:
                        LOG.error("Listing single profiles not permitted.")
                        raise
                    if not os.path.isdir(profile):
                        LOG.error("Profile location "+str(profile)+" is not a directory")
                        raise
                else:
                    raise
            else:
                ...
                if not os.path.isdir(profile):
                    LOG.error("Profile location "+str(profile)+" is not a directory. Has profiles.ini been tampered with?")
        ...
        def parse_sys_args() -> argparse.Namespace:
            ...
            parser.add_argument(
                ...
                action="store",
                default="",
                help=("Export username as is (default), or with the provided format prefix.\nFor instance 'login: ' for browserpass."))
            parser.add_argument(
                "-p", "--pass-prefix",
                action="store",
                default="web",
        ...
        def main() -> None:
            ...
            if args.encoding != DEFAULT_ENCODING:
                LOG.info("Overriding default encoding   from "+str(DEFAULT_ENCODING)+" to "+str(args.encoding))
                DEFAULT_ENCODING = args.encoding
            LOG.info("Running firefox_decrypt version: "+str(__version__))
            LOG.debug("Parsed commandline arguments: "+str(args))
            encodings = (
            ...
            for stream, encoding in encodings:
                if encoding.lower() != DEFAULT_ENCODING:
                    LOG.warning("Running with unsupported encoding "+str(stream)+": "+str(encoding)+" - Things are likely to fail from here onwards")
